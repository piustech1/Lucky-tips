...
import { Calendar, Filter, ArrowLeft, Trophy, CheckCircle2, XCircle } from 'lucide-react';
import { Link } from 'react-router-dom';
import { format, subDays } from 'date-fns';
import { Prediction } from '../types';
import { cn } from '../lib/utils';

const MOCK_HISTORY: Prediction[] = Array.from({ length: 10 }).map((_, i) => ({
  id: `hist-${i}`,
  date: subDays(new Date(), i + 1).toISOString(),
  league: i % 2 === 0 ? 'Champions League' : 'Bundesliga',
  homeTeam: i % 2 === 0 ? 'Liverpool' : 'Bayern Munich',
  awayTeam: i % 2 === 0 ? 'Real Madrid' : 'Dortmund',
  tip: i % 2 === 0 ? '1' : 'Over 2.5',
...
export default function PreviousPredictions() {
  ...
  const filtered = MOCK_HISTORY.filter(p => format(new Date(p.date), 'yyyy-MM-dd') === filterDate);
          ...
          >
            <div className="flex justify-between items-center mb-5">
              ...
              <div className={cn(
                ...
                {match.status === 'won' ? <CheckCircle2 className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
                {match.status}
              </div>
            </div>
