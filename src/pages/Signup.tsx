  ...
  return (
    <div className="min-h-screen bg-[var(--background)] flex flex-col font-sans max-w-md mx-auto relative shadow-2xl overflow-hidden">
      ...
      <div className="relative h-[30vh] w-full">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[var(--background)] via-[var(--background)]/20 to-transparent transition-colors duration-300" />
        
        <div className="absolute top-8 left-6 right-6 flex items-center justify-between z-20">
          <button 
            onClick={() => navigate(-1)}
            className="w-10 h-10 rounded-2xl bg-[var(--background)] border border-[var(--border)] flex items-center justify-center text-[var(--foreground)] hover:bg-[var(--muted)] transition-all shadow-lg"
          >
      ...
      <motion.div 
        ...
        className="flex-1 bg-[var(--card)] px-8 pt-2 pb-12 relative z-20 border-t border-[var(--border)] rounded-t-[40px] -mt-10 shadow-2xl overflow-y-auto"
      >
        <form onSubmit={handleSubmit} className="space-y-5 max-w-[340px] mx-auto">
          <div className="space-y-4 pt-6">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-widest ml-1 lowercase">Phone Number</label>
              <div className="flex gap-2">
                <div className="relative w-28 shrink-0 group">
                  <select 
                    value={formData.countryCode}
                    onChange={(e) => setFormData({...formData, countryCode: e.target.value})}
                    className="w-full h-14 bg-[var(--muted)] border-none rounded-2xl pl-3 pr-8 text-sm font-black appearance-none focus:ring-2 focus:ring-primary/20 transition-all text-[var(--foreground)]"
                  >
                    {COUNTRIES.map(c => (
                      <option key={c.code} value={c.code} className="bg-[var(--background)]">{c.flag} {c.code}</option>
                    ))}
                  </select>
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--muted-foreground)] group-hover:text-primary transition-colors">
                    <ChevronRight className="w-3.5 h-3.5 rotate-90" />
                  </div>
                </div>
                <div className="relative flex-1 group">
                  <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--muted-foreground)] group-focus-within:text-primary transition-colors" />
                  <input 
                    type="tel"
                    placeholder="701 234 567"
                    value={formData.phone}
                    onChange={(e) => setFormData({...formData, phone: e.target.value})}
                    className="w-full h-14 bg-[var(--muted)] border-none rounded-2xl pl-12 pr-4 text-sm font-black placeholder:text-[var(--muted-foreground)]/40 focus:ring-2 focus:ring-primary/20 transition-all text-[var(--foreground)]"
                    required
                  />
                </div>
              </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full h-14 bg-premium-gradient text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 mt-4"
          >
            {isLoading ? (
              <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            ) : (
              <>
                <span className="text-glow-teal">Create Secret Account</span>
          ...
          <div className="relative py-2">
            ...
            <div className="relative flex justify-center"><span className="px-3 bg-[var(--card)] text-[10px] font-black text-[var(--muted-foreground)]/50 uppercase tracking-widest lowercase tracking-tighter">or continue with</span></div>
          </div>
